'''
    DCT, quantization, masking, and ANS coding.
'''
...
def compress_image(image_path: str, output_path: str) -> None:
    ...
    metadata = {
        ...
        'words_length': len(encoded_signal.words),
        'bytes_length': len(encoded_bytes),
    }
...
def reconstruct_image(compressed_path: str) -> np.ndarray:
    ...
    words = np.frombuffer(
        ...
        dtype=np.uint32,
        count=int(metadata['words_length']),
    ).copy()
...
if __name__ == '__main__':
    ...
    ct.plot_zoomed_comparison(float_original, float_reconstructed, title="DCT ANS Compression")
